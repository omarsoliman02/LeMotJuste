...
\begin{center}
  ...
  {\large\bfseries Binôme : Omar Soliman \quad\textbullet\quad [Prénom NOM du binôme]}\\[4pt]
...
Ces listes proviennent de \textbf{données lexicales réelles} : la validation réunit les
dictionnaires Hunspell/Dicollecte et l'\textit{Officiel du Scrabble} (ODS8), soit environ
237\,000 mots ; les solutions sont des mots fréquents (corpus OpenSubtitles) recoupés avec
ce dictionnaire. Les deux listes sont régénérables par un script (\texttt{scripts/}).
...
fourni (dossier \texttt{k8s/}, appliqué par \texttt{kubectl apply -k k8s/}) : un
\texttt{Deployment} et un \texttt{Service} par composant, \texttt{Secret} pour les
identifiants PostgreSQL, \texttt{ConfigMap} pour les URLs et gateway exposée en \texttt{NodePort}.
