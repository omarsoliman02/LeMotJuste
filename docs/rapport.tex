...
\begin{center}
  ...
  {\small Dépôt GitHub : \url{https://github.com/omarsoliman02/LeMotJuste}}\\[2pt]
  {\small Juillet 2026}
\end{center}
\vspace{4pt}
...
% =====================================================================
Le c\oe{}ur du jeu est le calcul lettre par lettre d'une proposition. Pour gérer
...
La page d'accueil (avant de choisir son pseudo) a été enrichie d'éléments visuels non
textuels : une rangée de tuiles colorées façon Wordle en guise de bandeau, et trois
pictogrammes illustrant en un coup d'\oe{}il les règles (indice de départ, six essais,
code couleur). Un bouton \textbf{Règles} dans l'en-tête ouvre à tout moment une fenêtre
récapitulant les règles du jeu, sans quitter l'écran en cours.

Le joueur saisit ou choisit un nom, puis \textbf{choisit la taille de la grille}
(4 à 10 lettres, ou "Auto" pour un tirage aléatoire comme avant) avant de démarrer
une partie : ce choix est transmis à \texttt{POST /api/games} via le champ optionnel
\texttt{wordLength}, validé côté game-service par rapport aux bornes du dictionnaire.
La longueur du mot et sa première lettre sont affichées, cette dernière apparaissant en
gris clair sur la ligne en cours pour servir d'indice. À chaque proposition, la grille
se colore lettre par lettre (vert : bien placée ; jaune : mal placée ; gris : absente)
et un clavier AZERTY reflète l'état connu de chaque lettre. En fin de partie, le mot est
révélé ; une fenêtre de statistiques présente l'historique du joueur et le classement
(\texttt{/api/scores}). Les essais ne sont pas persistés côté serveur : la page
conserve les réponses reçues pour redessiner la grille.

Une \textbf{vue admin}, accessible depuis l'en-tête, agrège en lecture seule les
données des trois services (\texttt{GET /api/players}, l'endpoint
\texttt{GET /api/games} qui liste toutes les parties tous joueurs confondus avec
leur date de création, et \texttt{GET /api/scores}) : cartes de synthèse (nombre de
joueurs, de parties, de parties en cours, taux de victoire global), classement des
joueurs, et tableaux détaillés des parties et des scores, \textbf{filtrables par nom
de joueur et par période} (recherche côté client, sans appel réseau supplémentaire
puisque les données sont déjà chargées). L'accès est protégé par un mot de passe
saisi sur un écran de connexion dédié ; comme Spring Security est hors périmètre du
projet (cf. \S 8), cette protection reste côté client et ne remplace pas une
...
\url{http://localhost:5500}. Un déploiement sur \textbf{MiniKube} est également
disponible (\texttt{kubectl apply -k k8s/} : un \texttt{Deployment} et un
\texttt{Service} par composant, \texttt{Secret} pour les identifiants PostgreSQL,
\texttt{ConfigMap} pour les URLs inter-services, gateway exposée en
\texttt{NodePort}). Le guide détaillé, avec schémas d'architecture et script de démo,
est dans \texttt{docs/kubernetes-guide.md}.

% =====================================================================
\section{Bilan}
% =====================================================================
\textbf{Réussites.} Un découpage microservices clair et homogène (un même patron repris
pour chaque service), un contrat d'API figé qui a permis aux deux membres du binôme de travailler
